...
public class CallActivity extends LinphoneGenericActivity
    ...
    private TextView mContactName, mContactNumber, mContactCity, mMissedMessages;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContactCity = findViewById(R.id.contact_city);
    ...
    private void toggleMic() {
        ...
        mMicro.setSelected(!mCore.micEnabled());
        boolean mIsMicMuted = !mCore.micEnabled();
        if(mIsMicMuted) {
            mMicro.setImageResource(R.drawable.dial_filled_muted);
        } else {
            mMicro.setImageResource(R.drawable.mute_dial);
        }
    }
    ...
    private void toggleSpeaker() {
        ...
        mSpeaker.setSelected(mAudioManager.isAudioRoutedToSpeaker());
        boolean mIsSpeakerEnabled = mAudioManager.isAudioRoutedToSpeaker();
        if (mIsSpeakerEnabled) {
            mSpeaker.setImageResource(R.drawable.dial_speaker_filled);
        } else {
            mSpeaker.setImageResource(R.drawable.dial_speaker);
        }
    }
    ...
    private void togglePause(Call call) {
        ...
        if (call == mCore.getCurrentCall()) {
            ...
            mPause.setImageResource(R.drawable.dial_hold_filled);
        } else if (call.getState() == Call.State.Paused) {
            call.resume();
            mPause.setSelected(false);
            mPause.setImageResource(R.drawable.dial_hold_disabled);
        }
    }
...
//        }
        ...
        Address address = call.getRemoteAddress();
        LinphoneContact contact = ContactsManager.getInstance().findContactFromPhoneNumber(LinphoneUtils.getAddressDisplayName(address));
        if (contact != null) {
            ContactAvatar.displayAvatar(contact, findViewById(R.id.avatar_layout), true);
            mContactName.setText(contact.getFullName());
            mContactCity.setText(contact.getOrganization());
        } else {
            LinphoneContact mContact = ContactsManager.getInstance().findContactFromPhoneNumber("+234"+LinphoneUtils.getAddressDisplayName(address).substring(1));
            if(mContact != null) {
                ContactAvatar.displayAvatar(mContact, findViewById(R.id.avatar_layout), true);
                mContactName.setText(mContact.getFullName());
                mContactCity.setText(mContact.getOrganization());
            }else {
                String displayName = LinphoneUtils.getAddressDisplayName(address);
                ContactAvatar.displayAvatar(displayName, findViewById(R.id.avatar_layout), true);
                mContactName.setText(LinphoneUtils.getAddressDisplayName(address));
            }
        }
        mContactNumber.setText(LinphoneUtils.getAddressDisplayName(address));
    }
}
